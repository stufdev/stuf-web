...
import type { CardFilterOptions, CardFilters, CardStatistic, CardViewMode } from '@/lib/server/card-market-scanner';

type CardsFiltersProps = {
  filters: CardFilters;
  options: CardFilterOptions;
  isUpdating?: boolean;
  onFiltersChange: (nextValues: Partial<CardFilters>) => void;
};
...
export function CardsFilters({ filters, options, isUpdating = false, onFiltersChange }: CardsFiltersProps) {
  ...
  function updateQuery(nextValues: Partial<CardFilters>) {
  ...
  return (
    <section aria-busy={isBusy} className="border-y border-[var(--app-border)] bg-[var(--app-panel)] px-4 py-4">
          ...
          <select className="h-10 rounded border border-[var(--app-input-border)] bg-[var(--app-input-bg)] px-3 text-sm text-[var(--app-text)] outline-none focus:border-[var(--app-accent)]" disabled={isBusy} onChange={(event) => updateQuery({ line: event.target.value })} value={filters.line}>
            {lineOptions.map((line) => (
              <option key={line} value={line}>
                {filters.statistic === 'total_match_cards_under' ? `Under ${line}` : `Over ${line}`}
              </option>
            ))}
          </select>
        </label>
      ...
      <div className="mt-4 grid h-10 max-w-xl grid-cols-2 overflow-hidden rounded border border-[var(--app-input-border)] bg-[var(--app-input-bg)]">
        {([['all', 'All matches'], ['homeaway', 'Home/Away split']] satisfies Array<[CardViewMode, string]>).map(([value, label]) => {
