...
import type { CardStatSummary, CardTeamPanel, CardViewMode } from '@/lib/server/card-market-scanner';
import { CardMatchTable } from './card-match-table';
import { CardPercentageBar } from './card-percentage-bar';

type CardTeamPanelProps = {
  panel: CardTeamPanel;
...
function formatSample(stat: CardStatSummary | null) {
...
function SplitContextBadge({ label, stat }: { label: string; stat: CardStatSummary | null }) {
...
function ContextSplit({ panel }: { panel: CardTeamPanel }) {
  return (
...
function NextFixtureBlock({ panel }: { panel: CardTeamPanel }) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-[var(--app-border)] bg-[var(--app-panel-muted)] px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link
          ...
          href={`/comparison?fixtureId=${panel.nextFixture.fixtureId}&marketKey=${panel.marketKey}&source=cards`}
          prefetch={false}
        >
          Analyze Fixture
        </Link>
  ...
  return (
    <section className="rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
          ...
          <div className="min-w-0">
            ...
            {showBackToSummary ? (
              onBackToSummary ? (
                ...
                <Link
                  className="mt-2 inline-flex text-xs font-semibold text-[var(--app-accent)] hover:underline"
                  href={panel.summaryHref}
                  prefetch={false}
                >
                  Back to league summary
                </Link>
      ...
      <div className="grid gap-4 p-4">
        {viewMode === 'all' ? (
          <>
            <CardPercentageBar stat={panel.stats.overall} title="Overall" />
          ...
          <>
            <CardPercentageBar stat={panel.stats.overall} title="Overall summary" variant="compact" />
            <div className="grid gap-4 xl:grid-cols-2">
              <div className="grid gap-3">
                <CardPercentageBar stat={panel.stats.home} title="Home" />
                <CardMatchTable
                  isLoading={isEvidenceUpdating}
              ...
              <div className="grid gap-3">
                <CardPercentageBar stat={panel.stats.away} title="Away" />
                <CardMatchTable
                  isLoading={isEvidenceUpdating}
