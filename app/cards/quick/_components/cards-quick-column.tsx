import type { CardQuickColumn as CardQuickColumnData } from '@/lib/server/card-market-scanner';
import { CardsQuickRow } from './cards-quick-row';

type CardsQuickColumnProps = {
  column: CardQuickColumnData;
};

export function CardsQuickColumn({ column }: CardsQuickColumnProps) {
  return (
    <section className="min-w-0 overflow-hidden rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      {column.rows.length === 0 ? (
        ...
        <div>
          {column.rows.map((row) => (
            <CardsQuickRow key={`${row.scope}:${row.teamId}:${row.marketKey}`} row={row} />
          ))}
        </div>
      )}
    </section>
