...
import type { CardQuickMetric, CardQuickRow as CardQuickRowData, CardScope } from '@/lib/server/card-market-scanner';
import { PercentageMiniBar } from './percentage-mini-bar';

type CardsQuickRowProps = {
  row: CardQuickRowData;
};
...
function formatPercentage(metric: CardQuickMetric | null) {
...
function scopeLabel(scope: CardScope) {
  if (scope === 'home') {
    return 'home matches';
  }
...
export function CardsQuickRow({ row }: CardsQuickRowProps) {
  return (
