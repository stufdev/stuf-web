import type { ShotQuickColumn as ShotQuickColumnData } from '@/lib/server/shot-market-scanner';
import { ShotsQuickRow } from './shots-quick-row';

type ShotsQuickColumnProps = {
  column: ShotQuickColumnData;
};

export function ShotsQuickColumn({ column }: ShotsQuickColumnProps) {
  return (
    <section className="min-w-0 overflow-hidden rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      {column.rows.length === 0 ? (
        ...
        <div>
          {column.rows.map((row) => (
            <ShotsQuickRow key={`${row.scope}:${row.teamId}:${row.marketKey}`} row={row} />
          ))}
        </div>
      )}
    </section>
