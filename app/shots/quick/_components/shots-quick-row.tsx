...
import type { ShotQuickMetric, ShotQuickRow as ShotQuickRowData, ShotScope } from '@/lib/server/shot-market-scanner';
import { PercentageMiniBar } from './percentage-mini-bar';

type ShotsQuickRowProps = {
  row: ShotQuickRowData;
};
...
function formatPercentage(metric: ShotQuickMetric | null) {
...
function scopeLabel(scope: ShotScope) {
  if (scope === 'home') {
    return 'home matches';
  }
...
export function ShotsQuickRow({ row }: ShotsQuickRowProps) {
  return (
