...
import type { ShotStatSummary, ShotTeamPanel, ShotViewMode } from '@/lib/server/shot-market-scanner';
import { ShotMatchTable } from './shot-match-table';
import { ShotPercentageBar } from './shot-percentage-bar';

type ShotTeamPanelProps = {
  panel: ShotTeamPanel;
...
function formatSample(stat: ShotStatSummary | null) {
...
function SplitContextBadge({ label, stat }: { label: string; stat: ShotStatSummary | null }) {
...
function ContextSplit({ panel }: { panel: ShotTeamPanel }) {
  return (
...
function NextFixtureBlock({ panel }: { panel: ShotTeamPanel }) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-[var(--app-border)] bg-[var(--app-panel-muted)] px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link
          ...
          href={`/comparison?fixtureId=${panel.nextFixture.fixtureId}&marketKey=${panel.marketKey}&source=shots`}
        >
          Analyze Fixture
        </Link>
      </div>
  ...
  return (
    <section className="rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      <div className="grid gap-4 p-4">
        {viewMode === 'all' ? (
          <>
            <ShotPercentageBar stat={panel.stats.overall} title="Overall" />
          ...
          <>
            <ShotPercentageBar stat={panel.stats.overall} title="Overall summary" variant="compact" />
            <div className="grid gap-4 xl:grid-cols-2">
              <div className="grid gap-3">
                <ShotPercentageBar stat={panel.stats.home} title="Home" />
                <ShotMatchTable
                  isLoading={isEvidenceUpdating}
              ...
              <div className="grid gap-3">
                <ShotPercentageBar stat={panel.stats.away} title="Away" />
                <ShotMatchTable
                  isLoading={isEvidenceUpdating}
