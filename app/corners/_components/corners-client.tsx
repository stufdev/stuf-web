...
import type {
  ...
  CornerFilterOptions,
  CornerFilters,
  CornerMatchEvidenceResult,
} from '@/lib/server/corner-detail-scanner';
import { CornerTeamPanel } from './corner-team-panel';
import { CornersFilters } from './corners-filters';

type CornersClientProps = {
  initialFilters: CornerFilters;
  initialOptions: CornerFilterOptions;
  initialResult: CornerDetailResult;
};
...
type MatchEvidenceApiResponse =
  | {
      filters: CornerFilters;
      result: CornerMatchEvidenceResult;
      timing: {
        dbProfileMs: number;
        dbEvidenceMs: number;
        transformMs: number;
...
function cornerSearchParams(filters: CornerFilters) {
...
function selectedLeagueLabel(options: CornerFilterOptions, filters: CornerFilters) {
...
function getStatisticLabel(options: CornerFilterOptions, filters: CornerFilters) {
  return options.statistics.find((option) => option.value === filters.statistic)?.label ?? 'Corners';
}

function getLineLabel(filters: CornerFilters) {
  if (filters.statistic === 'corner_handicap') {
    return filters.line === '0' ? 'Most Corners' : `Handicap ${filters.line}`;
  }

  return `${filters.statistic === 'total_match_corners_under' ? 'Under' : 'Over'} ${filters.line}`;
}

function getPeriodGroupLabel(filters: CornerFilters) {
  return filters.periodGroup === 'by_half' ? 'By Half' : 'Full Match';
}

export function CornersClient({
  initialFilters,
  initialOptions,
  initialResult,
}: CornersClientProps) {
  ...
  async function loadEvidence(nextFilters: CornerFilters, baseResult: CornerDetailResult, requestId: number) {
    ...
    const evidenceParams = cornerSearchParams(nextFilters);
    evidenceParams.set('category', 'corners');
    ...
    try {
      ...
      if (!response.ok || 'error' in payload) {
        throw new Error('error' in payload ? payload.error : 'Corner evidence could not be loaded.');
      }
      ...
      if (requestSequence.current === requestId) {
        setErrorMessage(error instanceof Error ? error.message : 'Corner evidence could not be loaded.');
      }
    } finally {
      if (requestSequence.current === requestId) {
        setIsEvidenceUpdating(false);
  ...
  async function updateFilters(nextValues: Partial<CornerFilters>) {
    const nextFilters = { ...filters, ...nextValues };
    const currentParams = cornerSearchParams(filters);
    const nextParams = cornerSearchParams(nextFilters);
    ...
    try {
      ...
      if (!response.ok || 'error' in payload) {
        throw new Error('error' in payload ? payload.error : 'Corners could not be loaded.');
      }
      ...
      if (requestSequence.current === requestId) {
        setErrorMessage(error instanceof Error ? error.message : 'Corners could not be loaded.');
      }
    } finally {
      if (requestSequence.current === requestId) {
        setIsUpdating(false);
    ...
    <>
      <section className="px-4 py-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold tracking-normal">Corners</h1>
            <div className="mt-1 text-sm text-[var(--app-text-dim)]">Detailed corner market analysis</div>
            <div className="mt-2 flex flex-wrap gap-2 text-xs text-[var(--app-text-dim)]">
              ...
              <span>{getPeriodGroupLabel(filters)}</span>
      ...
      <section className="grid gap-4 px-4 py-4">
        ...
        {result.panels.length === 0 ? (
          <div className="border border-dashed border-[var(--app-border-strong)] bg-[var(--app-panel)] px-4 py-10 text-center">
            <div className="text-sm font-semibold text-[var(--app-text)]">No corner data for these filters.</div>
            <div className="mt-1 text-xs text-[var(--app-text-dim)]">
              Try another league, season, statistic, or line.
            </div>
          </div>
        ) : (
