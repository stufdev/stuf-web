...
import type { CornerStatSummary, CornerTeamPanel, CornerViewMode } from '@/lib/server/corner-detail-scanner';
import { CornerMatchTable } from './corner-match-table';
import { CornerPercentageBar } from './corner-percentage-bar';

type CornerTeamPanelProps = {
  panel: CornerTeamPanel;
...
function ContextSplit({ panel }: { panel: CornerTeamPanel }) {
  return (
...
function NextFixtureBlock({ panel }: { panel: CornerTeamPanel }) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-[var(--app-border)] bg-[var(--app-panel-muted)] px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link
          ...
          href={`/comparison?fixtureId=${panel.nextFixture.fixtureId}&marketKey=${panel.marketKey}&source=corners`}
        >
          Analyze Fixture
        </Link>
      </div>
...
export function CornerTeamPanel({
  panel,
  viewMode,
  ...
  return (
    <section className="rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      <div className="grid gap-4 p-4">
        {viewMode === 'all' ? (
          <>
            <CornerPercentageBar stat={panel.stats.overall} title="Overall" />
          ...
          <>
            <CornerPercentageBar stat={panel.stats.overall} title="Overall summary" variant="compact" />
            <div className="grid gap-4 xl:grid-cols-2">
              <div className="grid gap-3">
                <CornerPercentageBar stat={panel.stats.home} title="Home" />
                <CornerMatchTable
                  isLoading={isEvidenceUpdating}
              ...
              <div className="grid gap-3">
                <CornerPercentageBar stat={panel.stats.away} title="Away" />
                <CornerMatchTable
                  isLoading={isEvidenceUpdating}
