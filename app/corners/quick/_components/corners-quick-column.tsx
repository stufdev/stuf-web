import type { CornerQuickColumn as CornerQuickColumnData } from '@/lib/server/corner-quick-scanner';
import { CornersQuickRow } from './corners-quick-row';

type CornersQuickColumnProps = {
  column: CornerQuickColumnData;
};

export function CornersQuickColumn({ column }: CornersQuickColumnProps) {
  return (
    <section className="min-w-0 overflow-hidden rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      {column.rows.length === 0 ? (
        ...
        <div>
          {column.rows.map((row) => (
            <CornersQuickRow key={`${row.scope}:${row.teamId}:${row.marketKey}`} row={row} />
          ))}
        </div>
      )}
    </section>
