...
import type { CornerQuickMetric, CornerQuickRow, CornerQuickScope } from '@/lib/server/corner-quick-scanner';
import { PercentageMiniBar } from './percentage-mini-bar';

type CornersQuickRowProps = {
  row: CornerQuickRow;
};
...
function formatPercentage(metric: CornerQuickMetric | null) {
...
function scopeLabel(scope: CornerQuickScope) {
  if (scope === 'home') {
    return 'home matches';
  }
...
export function CornersQuickRow({ row }: CornersQuickRowProps) {
  return (
