import type { GoalMatchRow } from '@/lib/server/goal-market-scanner';

type GoalMatchTableProps = {
  isLoading?: boolean;
  rows: GoalMatchRow[];
  teamId: number;
  title: string;
...
function formatGoals(row: GoalMatchRow) {
  if (row.goalScoreLabel) return row.goalScoreLabel;
...
export function GoalMatchTable({ isLoading = false, rows, teamId, title }: GoalMatchTableProps) {
  if (isLoading) {
    return (
        ...
        <tbody className="divide-y divide-[var(--app-border)]">
          {rows.map((row) => (
            <tr className={row.result ? 'bg-emerald-500/10' : 'bg-red-500/10'} key={`${title}:${row.fixtureId}`}>
              ...
              <td className="px-3 py-2 font-semibold text-[var(--app-text)]">{formatGoals(row)}</td>
              <td className="px-3 py-2">
