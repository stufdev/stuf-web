import type { GoalQuickColumn as GoalQuickColumnData } from '@/lib/server/goal-market-scanner';
import { GoalsQuickRow } from './goals-quick-row';

type GoalsQuickColumnProps = {
  column: GoalQuickColumnData;
};

export function GoalsQuickColumn({ column }: GoalsQuickColumnProps) {
  return (
    <section className="min-w-0 overflow-hidden rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      {column.rows.length === 0 ? (
        ...
        <div>
          {column.rows.map((row) => (
            <GoalsQuickRow key={`${row.scope}:${row.teamId}:${row.marketKey}`} row={row} />
          ))}
        </div>
      )}
    </section>
