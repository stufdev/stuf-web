...
type GoalsQuickFiltersProps = {
  filters: GoalQuickFilters;
  options: GoalFilterOptions;
  isUpdating?: boolean;
  onFiltersChange?: (nextValues: Partial<GoalQuickFilters>) => void;
};

const fixtureTabs: Array<[GoalFixtureFilter, string]> = [
  ['all', 'All Teams'],
...
export function GoalsQuickFilters({
  filters,
  isUpdating = false,
  onFiltersChange,
  options,
}: GoalsQuickFiltersProps) {
  ...
  function updateQuery(nextValues: Partial<GoalQuickFilters>) {
    if (onFiltersChange) {
    ...
    if (nextMinSample >= 4) params.set('minSample', '4');
