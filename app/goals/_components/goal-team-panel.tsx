...
import type { GoalStatSummary, GoalTeamPanel as GoalTeamPanelData, GoalViewMode } from '@/lib/server/goal-market-scanner';
import { GoalMatchTable } from './goal-match-table';
import { GoalPercentageBar } from './goal-percentage-bar';

type GoalTeamPanelProps = {
  panel: GoalTeamPanelData;
...
function formatNextFixtureDate(value: string) {
  ...
  return `${weekdays[date.getDay()]} ${String(date.getDate()).padStart(2, '0')} ${months[date.getMonth()]} ${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`;
}
...
function formatSample(stat: GoalStatSummary | null) {
  if (!stat || stat.sample === 0) return '-';
  return `${stat.hits}/${stat.sample}`;
}

function SplitContextBadge({ label, stat }: { label: string; stat: GoalStatSummary | null }) {
...
function ContextSplit({ panel }: { panel: GoalTeamPanelData }) {
  return (
...
function NextFixtureBlock({ panel }: { panel: GoalTeamPanelData }) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-[var(--app-border)] bg-[var(--app-panel-muted)] px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link
          ...
          href={`/comparison?fixtureId=${panel.nextFixture.fixtureId}&marketKey=${panel.marketKey}&source=goals`}
        >
          Analyze Fixture
        </Link>
      </div>
  ...
  return (
    <section className="rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      <div className="grid gap-4 border-b border-[var(--app-border)] p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)]">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {showBackToSummary && onBackToSummary ? (
              <button
      ...
      <div className="grid gap-4 p-4">
        {viewMode === 'all' ? (
          <>
            <GoalPercentageBar stat={panel.stats.overall} title="Overall" />
            <ContextSplit panel={panel} />
            <GoalMatchTable isLoading={isEvidenceUpdating} rows={panel.matchRows.overall} teamId={panel.teamId} title="Overall" />
          </>
        ) : (
          <>
            <GoalPercentageBar stat={panel.stats.overall} title="Overall summary" variant="compact" />
            <div className="grid gap-4 xl:grid-cols-2">
              <div className="grid gap-3">
                <GoalPercentageBar stat={panel.stats.home} title="Home" />
                <GoalMatchTable isLoading={isEvidenceUpdating} rows={panel.matchRows.home} teamId={panel.teamId} title="Home" />
              </div>
              <div className="grid gap-3">
                <GoalPercentageBar stat={panel.stats.away} title="Away" />
                <GoalMatchTable isLoading={isEvidenceUpdating} rows={panel.matchRows.away} teamId={panel.teamId} title="Away" />
              </div>
            </div>
          </>
        )}
