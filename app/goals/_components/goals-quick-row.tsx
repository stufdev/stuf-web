...
import type { GoalQuickMetric, GoalQuickRow as GoalQuickRowData, GoalScope } from '@/lib/server/goal-market-scanner';
import { PercentageMiniBar } from './percentage-mini-bar';

type GoalsQuickRowProps = {
  row: GoalQuickRowData;
};
...
function formatPercentage(metric: GoalQuickMetric | null) {
  if (!metric || metric.sample === 0 || metric.percentage === null || !Number.isFinite(metric.percentage)) return '-';
...
export function GoalsQuickRow({ row }: GoalsQuickRowProps) {
  return (
