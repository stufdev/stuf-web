import type { OffsideMatchRow } from '@/lib/server/offside-market-scanner';

type OffsideMatchTableProps = {
  isLoading?: boolean;
...
function formatOffsides(row: OffsideMatchRow) {
  if (row.offsideScoreLabel) {
    return row.offsideScoreLabel;
  }

  const { homeOffsides, awayOffsides } = row;
  if (homeOffsides === null || awayOffsides === null) {
    return '-';
  }

  return `${homeOffsides} - ${awayOffsides}`;
}
...
export function OffsideMatchTable({ isLoading = false, rows, teamId, title }: OffsideMatchTableProps) {
  if (isLoading) {
    return (
      ...
      <table className="min-w-[720px] text-sm">
        <thead className="bg-[var(--app-panel-muted)] text-xs uppercase text-[var(--app-text-dim)]">
          <tr>
            ...
            <th className="px-3 py-2 text-left">Offsides</th>
        ...
        <tbody className="divide-y divide-[var(--app-border)]">
          {rows.map((row) => (
            <tr className={row.result ? 'bg-emerald-500/10' : 'bg-red-500/10'} key={`${title}:${row.fixtureId}`}>
              ...
              <td className="px-3 py-2 font-semibold text-[var(--app-text)]">{formatOffsides(row)}</td>
              <td className="px-3 py-2">
