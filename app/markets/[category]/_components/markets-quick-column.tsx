import type { GenericQuickColumn as GenericQuickColumnData } from '@/lib/server/generic-market-scanner';
import { MarketsQuickRow } from './markets-quick-row';

type MarketsQuickColumnProps = {
  column: GenericQuickColumnData;
};

export function MarketsQuickColumn({ column }: MarketsQuickColumnProps) {
  return (
    <section className="min-w-0 overflow-hidden rounded border border-[var(--app-border)] bg-[var(--app-panel)]">
      ...
      {column.rows.length === 0 ? (
        ...
        <div>
          {column.rows.map((row) => (
            <MarketsQuickRow key={`${row.scope}:${row.teamId}:${row.marketKey}`} row={row} />
          ))}
        </div>
      )}
    </section>
