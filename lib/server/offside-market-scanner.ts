...
export type OffsideStatistic =
  | 'total_match_offsides'
  | 'total_match_offsides_under'
  | 'team_offsides_for'
  | 'team_offsides_against';
export type OffsideFixtureFilter = 'all' | 'with_fixture' | 'today' | 'tomorrow' | 'in_2_days';
export type OffsideFormWindow = 'season' | 'last5' | 'last10';
export type OffsideSearchParams = Record<string, string | string[] | undefined>;

export type OffsideFilters = {
  leagueId: number;
  season: number;
  statistic: OffsideStatistic;
  line: string;
...
export type OffsideQuickFilters = {
  leagueId: number;
  season: number;
  statistic: OffsideStatistic;
  line: string;
  teamSearch: string;
  formWindow: OffsideFormWindow;
  fixtureFilter: OffsideFixtureFilter;
  minSample: number;
};

export type OffsideLeagueSeasonOption = {
  leagueId: number;
  leagueName: string;
...
export type OffsideStatisticOption = {
  value: OffsideStatistic;
  label: string;
};

export type OffsideFilterOptions = {
  defaultLeagueId: number;
  defaultSeason: number;
  leagueSeasons: OffsideLeagueSeasonOption[];
  linesByStatistic: Record<OffsideStatistic, string[]>;
  statistics: OffsideStatisticOption[];
};

export type OffsideStatSummary = {
  scope: OffsideScope;
  sample: number;
  hits: number;
  percentage: number;
...
export type OffsideMatchRow = {
  ...
  offsideScoreLabel: string | null;
  numericValue: number | null;
  result: boolean;
};

export type OffsideNextFixture = {
  fixtureId: number;
...
export type OffsideTeamPanel = {
  ...
  marketKey: string;
  marketLabel: string;
  stats: Record<OffsideScope, OffsideStatSummary | null>;
  matchRows: Record<OffsideScope, OffsideMatchRow[]>;
  nextFixture: OffsideNextFixture | null;
  evidenceLoaded: boolean;
  evidenceHref: string;
  summaryHref: string;
};

export type OffsideDetailResult = {
  marketKey: string;
  marketLabel: string;
  marketAvailable: boolean;
  evidenceMode: 'summary' | 'evidence';
  panels: OffsideTeamPanel[];
};

export type OffsideDetailTiming = {
  dbProfileMs: number;
  dbEvidenceMs: number;
  transformMs: number;
  totalMs: number;
...
export type OffsideTeamEvidenceGroup = {
  teamId: number;
  scope: OffsideScope;
  rows: OffsideMatchRow[];
};

export type OffsideMatchEvidenceResult = {
  marketKey: string;
  marketLabel: string;
  groups: OffsideTeamEvidenceGroup[];
};

export type OffsideQuickMetric = {
  hits: number;
  sample: number;
...
export type OffsideQuickRow = {
  ...
  marketKey: string;
  marketLabel: string;
  metric: OffsideQuickMetric;
  nextFixture: OffsideNextFixture | null;
  opponentSupport: OffsideQuickMetric | null;
  opponentSupportScope: OffsideScope | null;
  fixtureContextMismatch: boolean;
  detailedHref: string;
  comparisonHref: string | null;
};

export type OffsideQuickColumn = {
  scope: OffsideScope;
  title: string;
  rows: OffsideQuickRow[];
};

export type OffsideQuickResult = {
  marketKey: string;
  marketLabel: string;
  marketAvailable: boolean;
...
type MarketTeamRankingRow = {
  ...
  next_opponent_name: string | null;
  next_venue_scope: 'home' | 'away' | null;
  opponent_support_scope: OffsideScope | null;
...
const MAX_OFFSIDE_TEAM_PANELS = 40;
const QUICK_RESULT_LIMIT_PER_SCOPE = 60;
const MARKET_SERVING_CACHE_SECONDS = 60;
const SCOPES: OffsideScope[] = ['overall', 'home', 'away'];
...
export const OFFSIDE_STATISTIC_OPTIONS: OffsideStatisticOption[] = [
  { value: 'total_match_offsides', label: 'Total Match Offsides Over' },
  { value: 'total_match_offsides_under', label: 'Total Match Offsides Under' },
  { value: 'team_offsides_for', label: 'Total Team Offsides For' },
  { value: 'team_offsides_against', label: 'Total Team Offsides Against' },
];

export const OFFSIDE_LINES_BY_STATISTIC: Record<OffsideStatistic, string[]> = {
  total_match_offsides: ['0.5', '1.5', '2.5', '3.5', '4.5', '5.5'],
  total_match_offsides_under: ['0.5', '1.5', '2.5', '3.5', '4.5', '5.5'],
  team_offsides_for: ['0.5', '1.5', '2.5', '3.5'],
  team_offsides_against: ['0.5', '1.5', '2.5', '3.5'],
};

const DEFAULT_LINE_BY_STATISTIC: Record<OffsideStatistic, string> = {
  total_match_offsides: '2.5',
  total_match_offsides_under: '2.5',
  team_offsides_for: '1.5',
  team_offsides_against: '1.5',
};
...
function parseStatistic(value: string | string[] | undefined): OffsideStatistic {
  const raw = firstParam(value);
  return OFFSIDE_STATISTIC_OPTIONS.find((item) => item.value === raw)?.value ?? 'total_match_offsides';
}

function parseViewMode(value: string | string[] | undefined): OffsideViewMode {
  return firstParam(value) === 'homeaway' ? 'homeaway' : 'all';
}

function parseFormWindow(value: string | string[] | undefined): OffsideFormWindow {
  const raw = firstParam(value);
  return raw === 'last5' || raw === 'last10' ? raw : 'season';
}

function parseFixtureFilter(value: string | string[] | undefined): OffsideFixtureFilter {
  const raw = firstParam(value);
...
function hasLeagueSeason(options: OffsideFilterOptions, leagueId: number, season: number) {
...
export function normalizeOffsideLine(statistic: OffsideStatistic, value: string | undefined) {
  const lines = OFFSIDE_LINES_BY_STATISTIC[statistic];
  const parsed = Number(value);
  return lines.find((line) => Number(line) === parsed) ?? DEFAULT_LINE_BY_STATISTIC[statistic];
}

export function resolveOffsideMarketKey(statistic: OffsideStatistic, line: string) {
  const keyLine = lineKey(normalizeOffsideLine(statistic, line));
  if (statistic === 'total_match_offsides') return `MATCH_OVER_${keyLine}_OFFSIDES`;
  if (statistic === 'total_match_offsides_under') return `MATCH_UNDER_${keyLine}_OFFSIDES`;
  if (statistic === 'team_offsides_for') return `TEAM_OVER_${keyLine}_OFFSIDES_FOR`;
  return `TEAM_OVER_${keyLine}_OFFSIDES_AGAINST`;
}

export function statisticFromOffsideMarketKey(marketKey: string): { statistic: OffsideStatistic; line: string } | null {
  const total = marketKey.match(/^MATCH_(OVER|UNDER)_([0-9]+)_([0-9]+)_OFFSIDES$/);
  if (total) return { statistic: total[1] === 'UNDER' ? 'total_match_offsides_under' : 'total_match_offsides', line: `${total[2]}.${total[3]}` };
  const teamFor = marketKey.match(/^TEAM_OVER_([0-9]+)_([0-9]+)_OFFSIDES_FOR$/);
  if (teamFor) return { statistic: 'team_offsides_for', line: `${teamFor[1]}.${teamFor[2]}` };
  const teamAgainst = marketKey.match(/^TEAM_OVER_([0-9]+)_([0-9]+)_OFFSIDES_AGAINST$/);
  if (teamAgainst) return { statistic: 'team_offsides_against', line: `${teamAgainst[1]}.${teamAgainst[2]}` };
  return null;
}

function fallbackMarketLabel(statistic: OffsideStatistic, line: string) {
  const option = OFFSIDE_STATISTIC_OPTIONS.find((item) => item.value === statistic);
  const operator = statistic === 'total_match_offsides_under' ? 'Under' : 'Over';
  return `${operator} ${normalizeOffsideLine(statistic, line)} ${option?.label.replace(/ Over| Under/u, '') ?? 'Offsides'}`;
}

function allOffsideMarketKeys() {
  return OFFSIDE_STATISTIC_OPTIONS.flatMap((option) => (
    OFFSIDE_LINES_BY_STATISTIC[option.value].map((line) => resolveOffsideMarketKey(option.value, line))
  ));
}
...
function isOffsideScope(value: unknown): value is OffsideScope {
...
function toRankingRows(rows: unknown[]): MarketTeamRankingRow[] {
  return rows.flatMap((row) => {
    ...
    return [{
      ...
      next_opponent_name: typeof record.next_opponent_name === 'string' ? record.next_opponent_name : null,
      next_venue_scope: record.next_venue_scope === 'home' || record.next_venue_scope === 'away' ? record.next_venue_scope : null,
      opponent_support_scope: isOffsideScope(record.opponent_support_scope) ? record.opponent_support_scope : null,
...
function toSnapshotSummary(row: TeamMarketProfileRow, scope: OffsideScope): OffsideStatSummary | null {
...
function emptyMatchRows(): Record<OffsideScope, OffsideMatchRow[]> {
  return { overall: [], home: [], away: [] };
}

function matchScopesForViewMode(viewMode: OffsideViewMode): OffsideScope[] {
  return viewMode === 'homeaway' ? ['home', 'away'] : ['overall'];
}

function toOffsideMatchRow(row: TeamMarketMatchEvidenceRow): OffsideMatchRow {
  const homeOffsides = toFiniteNumber(row.home_value);
  const awayOffsides = toFiniteNumber(row.away_value);
  return {
    ...
    homeOffsides,
    awayOffsides,
    offsideScoreLabel: homeOffsides === null || awayOffsides === null ? null : `${homeOffsides} - ${awayOffsides}`,
    numericValue: null,
    result: row.result,
  };
}

function groupEvidenceRows(evidenceRows: TeamMarketMatchEvidenceRow[], viewMode: OffsideViewMode): OffsideTeamEvidenceGroup[] {
  const allowedScopes = new Set(matchScopesForViewMode(viewMode));
  const rowsByTeamScope = new Map<string, OffsideMatchRow[]>();
  for (const row of evidenceRows) {
    if (!allowedScopes.has(row.scope)) continue;
    const key = `${row.team_id}:${row.scope}`;
    const currentRows = rowsByTeamScope.get(key) ?? [];
    currentRows.push(toOffsideMatchRow(row));
    rowsByTeamScope.set(key, currentRows);
  }
  return [...rowsByTeamScope.entries()].map(([key, rows]) => {
    const [teamId, scope] = key.split(':');
    return { teamId: Number(teamId), scope: scope as OffsideScope, rows };
  });
}

function evidenceGroupsToMap(groups: OffsideTeamEvidenceGroup[]) {
  return new Map(groups.map((group) => [`${group.teamId}:${group.scope}`, group.rows]));
}

function countReturnedEvidenceRows(groups: OffsideTeamEvidenceGroup[]) {
  return groups.reduce((total, group) => total + group.rows.length, 0);
}

function buildOffsidesHref(filters: OffsideFilters, teamId: number | null) {
  ...
  return `/offsides?${params.toString()}`;
}

function toOffsideTeamPanel({
  evidenceLoaded,
  filters,
  marketKey,
  marketLabel,
  ...
  marketKey: string;
  marketLabel: string;
  matchRowsByTeamScope: Map<string, OffsideMatchRow[]>;
  ...
  return {
    ...
    nextFixture: row.next_fixture_id !== null && row.next_fixture_date !== null && row.next_opponent_team_id !== null && row.next_opponent_name !== null && row.next_venue_scope !== null
      ...
      : null,
    evidenceLoaded,
    evidenceHref: buildOffsidesHref(filters, row.team_id),
    summaryHref: buildOffsidesHref({ ...filters, teamSearch: '', teamId: null }, null),
  };
}

function metricFromQuickSnapshot(row: MarketTeamRankingRow, formWindow: OffsideFormWindow): OffsideQuickMetric {
  if (formWindow === 'last5') {
...
function opponentMetricFromQuickSnapshot(row: MarketTeamRankingRow, formWindow: OffsideFormWindow): OffsideQuickMetric | null {
...
function venueMatchesScope(scope: OffsideScope, fixture: OffsideNextFixture | null) {
  if (!fixture || scope === 'overall') return true;
  return scope === 'home' ? fixture.isHome : !fixture.isHome;
}

function fixtureMatchesFilter(scope: OffsideScope, fixture: OffsideNextFixture | null, fixtureFilter: OffsideFixtureFilter) {
  ...
  const targets: Record<Exclude<OffsideFixtureFilter, 'all' | 'with_fixture'>, string> = {
    today,
...
function buildComparisonHref(fixtureId: number, marketKey: string) {
  return `/comparison?fixtureId=${fixtureId}&marketKey=${marketKey}&source=offsides-quick`;
}

function scopeTitle(scope: OffsideScope) {
  if (scope === 'home') return 'Home Matches';
  if (scope === 'away') return 'Away Matches';
  return 'All Matches';
}

export function parseOffsideFilters(searchParams: OffsideSearchParams | undefined, options: OffsideFilterOptions): OffsideFilters {
  const marketKeySelection = statisticFromOffsideMarketKey(firstParam(searchParams?.marketKey) ?? '');
  ...
  return {
    ...
    season,
    statistic,
    line: normalizeOffsideLine(statistic, marketKeySelection?.line ?? firstParam(searchParams?.line)),
...
export function parseOffsideQuickFilters(searchParams: OffsideSearchParams | undefined, options: OffsideFilterOptions): OffsideQuickFilters {
  const marketKeySelection = statisticFromOffsideMarketKey(firstParam(searchParams?.marketKey) ?? '');
  const statistic = marketKeySelection?.statistic ?? parseStatistic(searchParams?.statistic);
  const requestedLeagueId = parseInteger(searchParams?.leagueId) ?? options.defaultLeagueId;
  const requestedSeason = parseInteger(searchParams?.season) ?? options.defaultSeason;
...
const loadMarketDefinition = unstable_cache(async (marketKey: string) => {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw new Error(`Failed to load offside market definition: ${error.message}`);
  return toMarketDefinitionRows(data ? [data] : [])[0] ?? null;
}, ['offside-market-definition'], { revalidate: 300 });

async function loadOffsideFilterOptionsUncached(): Promise<OffsideFilterOptions> {
  const supabaseAdmin = getSupabaseAdmin();
  const [{ data: supportedData, error: supportedError }, { data: marketData, error: marketError }] = await Promise.all([
    ...
    supabaseAdmin
      .from('market_definitions')
      .select('key, label, is_active')
      .in('key', allOffsideMarketKeys())
      .eq('category', 'offsides'),
  ]);
  if (supportedError) throw new Error(`Failed to load offside league options: ${supportedError.message}`);
  if (marketError) throw new Error(`Failed to load offside market options: ${marketError.message}`);
  ...
  const { data: leagueData, error: leagueError } = await supabaseAdmin
    ...
    .in('id', leagueIds);
  if (leagueError) throw new Error(`Failed to load offside leagues: ${leagueError.message}`);
  ...
  const linesByStatistic = Object.fromEntries(
    OFFSIDE_STATISTIC_OPTIONS.map((option) => [
      option.value,
      OFFSIDE_LINES_BY_STATISTIC[option.value].filter((line) => activeKeys.has(resolveOffsideMarketKey(option.value, line))),
    ]),
  ) as Record<OffsideStatistic, string[]>;

  for (const option of OFFSIDE_STATISTIC_OPTIONS) {
  ...
  return {
    ...
    leagueSeasons,
    linesByStatistic,
    statistics: OFFSIDE_STATISTIC_OPTIONS,
  };
}

export const loadOffsideFilterOptions = unstable_cache(loadOffsideFilterOptionsUncached, ['offside-filter-options'], { revalidate: 300 });

const loadOffsideProfileSnapshotRows = unstable_cache(
  async (marketKey: string, leagueId: number, season: number, teamId: number | null, teamSearch: string) => {
    ...
    const { data, error } = await query
      ...
      .limit(MAX_OFFSIDE_TEAM_PANELS);

    if (error) {
      throw new Error(`Failed to load offside Market Serving Layer profiles. Run rebuild_market_serving_layer.py --category offsides. Supabase: ${error.message}`);
    }

    return toProfileRows((data ?? []) as unknown[]);
  },
  ['offside-profile-snapshot-rows'],
  { revalidate: MARKET_SERVING_CACHE_SECONDS },
);

const loadOffsideEvidenceSnapshotRows = unstable_cache(
  async (marketKey: string, leagueId: number, season: number, viewMode: OffsideViewMode, teamIdsKey: string) => {
    ...
    if (error) {
      throw new Error(`Failed to load offside Market Serving Layer evidence. Run rebuild_market_serving_layer.py --category offsides. Supabase: ${error.message}`);
    }

    return toEvidenceRows((data ?? []) as unknown[]);
  },
  ['offside-evidence-snapshot-rows'],
  { revalidate: MARKET_SERVING_CACHE_SECONDS },
);

const loadOffsideQuickRankingSnapshotRows = unstable_cache(
  async (marketKey: string, leagueId: number, season: number) => {
    ...
    if (error) {
      throw new Error(`Failed to load offside quick Market Serving Layer. Run rebuild_market_serving_layer.py --category offsides. Supabase: ${error.message}`);
    }

    return toRankingRows((data ?? []) as unknown[]);
  },
  ['offside-quick-ranking-snapshot-rows'],
  { revalidate: MARKET_SERVING_CACHE_SECONDS },
);

export async function loadOffsideMatchEvidence(
  filters: OffsideFilters,
  teamIds: number[],
): Promise<{ result: OffsideMatchEvidenceResult; timing: OffsideDetailTiming }> {
  const startedAt = Date.now();
  const marketKey = resolveOffsideMarketKey(filters.statistic, filters.line);
  ...
  const evidenceRows = await loadOffsideEvidenceSnapshotRows(
    marketKey,
    filters.leagueId,
    filters.season,
    filters.viewMode,
...
export async function loadOffsideTeamPanelsWithTiming(
  filters: OffsideFilters,
  options: { includeEvidence?: boolean } = {},
): Promise<{ result: OffsideDetailResult; timing: OffsideDetailTiming }> {
  const startedAt = Date.now();
  const marketKey = resolveOffsideMarketKey(filters.statistic, filters.line);
  ...
  const [marketDefinition, profileRows] = await Promise.all([
    ...
    loadOffsideProfileSnapshotRows(marketKey, filters.leagueId, filters.season, filters.teamId, filters.teamSearch.trim()),
  ]);
  ...
  let evidenceGroups: OffsideTeamEvidenceGroup[] = [];
  let dbEvidenceMs = 0;
  let evidenceRowCount = 0;
  let returnedEvidenceRowCount = 0;

  if (includeEvidence) {
    const evidence = await loadOffsideMatchEvidence(filters, profileRows.map((row) => row.team_id));
  ...
  const panels = profileRows.map((row) => toOffsideTeamPanel({
...
export async function loadOffsideTeamPanels(filters: OffsideFilters): Promise<OffsideDetailResult> {
  const { result } = await loadOffsideTeamPanelsWithTiming(filters);
  return result;
}

export async function loadOffsideQuickScanner(filters: OffsideQuickFilters): Promise<OffsideQuickResult> {
  const { result } = await loadOffsideQuickScannerWithTiming(filters);
  return result;
}

export async function loadOffsideQuickScannerWithTiming(
  filters: OffsideQuickFilters,
): Promise<{ result: OffsideQuickResult; timing: OffsideQuickTiming }> {
  const startedAt = Date.now();
  const marketKey = resolveOffsideMarketKey(filters.statistic, filters.line);
  const dbStartedAt = Date.now();
  const [marketDefinition, snapshotRows] = await Promise.all([
    loadMarketDefinition(marketKey),
    loadOffsideQuickRankingSnapshotRows(marketKey, filters.leagueId, filters.season),
  ]);
