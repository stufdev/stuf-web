...
export type CardScope = 'overall' | 'home' | 'away';
export type CardViewMode = 'all' | 'homeaway';
export type CardStatistic =
  | 'total_match_cards'
  | 'total_match_cards_under'
  | 'team_cards_for'
  | 'team_cards_against'
  | 'each_team_cards';
export type CardFixtureFilter = 'all' | 'with_fixture' | 'today' | 'tomorrow' | 'in_2_days';
export type CardFormWindow = 'season' | 'last5' | 'last10';
...
export type CardQuickFilters = {
  leagueId: number;
  season: number;
  statistic: CardStatistic;
  line: string;
  teamSearch: string;
  formWindow: CardFormWindow;
  fixtureFilter: CardFixtureFilter;
  minSample: number;
};

export type CardLeagueSeasonOption = {
  leagueId: number;
  leagueName: string;
...
export type CardStatisticOption = {
  value: CardStatistic;
  label: string;
};
...
export type CardFilterOptions = {
  ...
  leagueSeasons: CardLeagueSeasonOption[];
  linesByStatistic: Record<CardStatistic, string[]>;
  statistics: CardStatisticOption[];
};

export type CardStatSummary = {
  scope: CardScope;
  sample: number;
  hits: number;
  percentage: number;
...
export type CardMatchRow = {
  ...
  cardScoreLabel: string | null;
  numericValue: number | null;
  result: boolean;
};

export type CardNextFixture = {
  fixtureId: number;
...
export type CardTeamPanel = {
  ...
  marketKey: string;
  marketLabel: string;
  stats: Record<CardScope, CardStatSummary | null>;
  matchRows: Record<CardScope, CardMatchRow[]>;
  nextFixture: CardNextFixture | null;
...
export type CardDetailResult = {
  ...
  panels: CardTeamPanel[];
};

export type CardDetailTiming = {
  dbProfileMs: number;
  dbEvidenceMs: number;
  transformMs: number;
  totalMs: number;
...
export type CardTeamEvidenceGroup = {
  teamId: number;
  scope: CardScope;
  rows: CardMatchRow[];
};

export type CardMatchEvidenceResult = {
  marketKey: string;
  marketLabel: string;
  groups: CardTeamEvidenceGroup[];
};

export type CardQuickMetric = {
  hits: number;
  sample: number;
...
export type CardQuickRow = {
  ...
  marketKey: string;
  marketLabel: string;
  metric: CardQuickMetric;
  nextFixture: CardNextFixture | null;
  opponentSupport: CardQuickMetric | null;
  opponentSupportScope: CardScope | null;
  fixtureContextMismatch: boolean;
  detailedHref: string;
  comparisonHref: string | null;
};

export type CardQuickColumn = {
  scope: CardScope;
  title: string;
  rows: CardQuickRow[];
};

export type CardQuickResult = {
  marketKey: string;
  marketLabel: string;
  marketAvailable: boolean;
  columns: CardQuickColumn[];
  emergingColumns: CardQuickColumn[];
};

export type CardQuickTiming = {
  dbMs: number;
  transformMs: number;
  totalMs: number;
  rowCount: number;
...
type MarketTeamRankingRow = {
  ...
  next_opponent_name: string | null;
  next_venue_scope: 'home' | 'away' | null;
  opponent_support_scope: CardScope | null;
...
export const CARD_STATISTIC_OPTIONS: CardStatisticOption[] = [
  { value: 'total_match_cards', label: 'Total Match Cards Over' },
  { value: 'total_match_cards_under', label: 'Total Match Cards Under' },
  { value: 'team_cards_for', label: 'Team Cards For' },
  { value: 'team_cards_against', label: 'Team Cards Against' },
  { value: 'each_team_cards', label: 'Each Team Cards' },
];

export const CARD_LINES_BY_STATISTIC: Record<CardStatistic, string[]> = {
  total_match_cards: ['1.5', '2.5', '3.5', '4.5', '5.5', '6.5'],
  total_match_cards_under: ['1.5', '2.5', '3.5', '4.5', '5.5', '6.5'],
  team_cards_for: ['0.5', '1.5', '2.5', '3.5'],
  team_cards_against: ['0.5', '1.5', '2.5', '3.5'],
  each_team_cards: ['0.5', '1.5', '2.5'],
};

const DEFAULT_LINE_BY_STATISTIC: Record<CardStatistic, string> = {
  total_match_cards: '3.5',
  total_match_cards_under: '3.5',
  team_cards_for: '1.5',
  team_cards_against: '1.5',
  each_team_cards: '0.5',
};
...
function parseStatistic(value: string | string[] | undefined): CardStatistic {
  const raw = firstParam(value);
  return CARD_STATISTIC_OPTIONS.find((item) => item.value === raw)?.value ?? 'total_match_cards';
}

function parseViewMode(value: string | string[] | undefined): CardViewMode {
  return firstParam(value) === 'homeaway' ? 'homeaway' : 'all';
}

function parseFormWindow(value: string | string[] | undefined): CardFormWindow {
  const raw = firstParam(value);
  return raw === 'last5' || raw === 'last10' ? raw : 'season';
}

function parseFixtureFilter(value: string | string[] | undefined): CardFixtureFilter {
  const raw = firstParam(value);
...
function hasLeagueSeason(options: CardFilterOptions, leagueId: number, season: number) {
...
export function normalizeCardLine(statistic: CardStatistic, value: string | undefined) {
  const lines = CARD_LINES_BY_STATISTIC[statistic];
  const parsed = Number(value);
  return lines.find((line) => Number(line) === parsed) ?? DEFAULT_LINE_BY_STATISTIC[statistic];
}

export function resolveCardMarketKey(statistic: CardStatistic, line: string) {
  const keyLine = lineKey(normalizeCardLine(statistic, line));
  if (statistic === 'total_match_cards') return `MATCH_OVER_${keyLine}_CARDS`;
  if (statistic === 'total_match_cards_under') return `MATCH_UNDER_${keyLine}_CARDS`;
  if (statistic === 'team_cards_for') return `TEAM_OVER_${keyLine}_CARDS_FOR`;
  if (statistic === 'each_team_cards') return `EACH_TEAM_OVER_${keyLine}_CARDS`;
  return `TEAM_OVER_${keyLine}_CARDS_AGAINST`;
}

export function statisticFromCardMarketKey(marketKey: string): { statistic: CardStatistic; line: string } | null {
  const total = marketKey.match(/^MATCH_(OVER|UNDER)_([0-9]+)_([0-9]+)_CARDS$/);
  if (total) return { statistic: total[1] === 'UNDER' ? 'total_match_cards_under' : 'total_match_cards', line: `${total[2]}.${total[3]}` };
  const teamFor = marketKey.match(/^TEAM_OVER_([0-9]+)_([0-9]+)_CARDS_FOR$/);
  if (teamFor) return { statistic: 'team_cards_for', line: `${teamFor[1]}.${teamFor[2]}` };
  const teamAgainst = marketKey.match(/^TEAM_OVER_([0-9]+)_([0-9]+)_CARDS_AGAINST$/);
  if (teamAgainst) return { statistic: 'team_cards_against', line: `${teamAgainst[1]}.${teamAgainst[2]}` };
  const eachTeam = marketKey.match(/^EACH_TEAM_OVER_([0-9]+)_([0-9]+)_CARDS$/);
  if (eachTeam) return { statistic: 'each_team_cards', line: `${eachTeam[1]}.${eachTeam[2]}` };
  return null;
}

function fallbackMarketLabel(statistic: CardStatistic, line: string) {
  const option = CARD_STATISTIC_OPTIONS.find((item) => item.value === statistic);
  const operator = statistic === 'total_match_cards_under' ? 'Under' : 'Over';
  return `${operator} ${normalizeCardLine(statistic, line)} ${option?.label.replace(/ Over| Under/u, '') ?? 'Cards'}`;
}

function allCardMarketKeys() {
  return CARD_STATISTIC_OPTIONS.flatMap((option) => (
    CARD_LINES_BY_STATISTIC[option.value].map((line) => resolveCardMarketKey(option.value, line))
  ));
}
...
function isCardScope(value: unknown): value is CardScope {
...
function toRankingRows(rows: unknown[]): MarketTeamRankingRow[] {
  return rows.flatMap((row) => {
    ...
    return [{
      ...
      next_opponent_name: typeof record.next_opponent_name === 'string' ? record.next_opponent_name : null,
      next_venue_scope: record.next_venue_scope === 'home' || record.next_venue_scope === 'away' ? record.next_venue_scope : null,
      opponent_support_scope: isCardScope(record.opponent_support_scope) ? record.opponent_support_scope : null,
...
function toSnapshotSummary(row: TeamMarketProfileRow, scope: CardScope): CardStatSummary | null {
...
function emptyMatchRows(): Record<CardScope, CardMatchRow[]> {
  return { overall: [], home: [], away: [] };
}

function matchScopesForViewMode(viewMode: CardViewMode): CardScope[] {
  return viewMode === 'homeaway' ? ['home', 'away'] : ['overall'];
}

function toCardMatchRow(row: TeamMarketMatchEvidenceRow): CardMatchRow {
  const homeCards = toFiniteNumber(row.home_value);
  const awayCards = toFiniteNumber(row.away_value);
  return {
    ...
    homeCards,
    awayCards,
    cardScoreLabel: homeCards === null || awayCards === null ? null : `${homeCards} - ${awayCards}`,
    numericValue: null,
    result: row.result,
  };
}

function groupEvidenceRows(evidenceRows: TeamMarketMatchEvidenceRow[], viewMode: CardViewMode): CardTeamEvidenceGroup[] {
  const allowedScopes = new Set(matchScopesForViewMode(viewMode));
  const rowsByTeamScope = new Map<string, CardMatchRow[]>();
  for (const row of evidenceRows) {
    if (!allowedScopes.has(row.scope)) continue;
    const key = `${row.team_id}:${row.scope}`;
    const currentRows = rowsByTeamScope.get(key) ?? [];
    currentRows.push(toCardMatchRow(row));
    rowsByTeamScope.set(key, currentRows);
  }
  return [...rowsByTeamScope.entries()].map(([key, rows]) => {
    const [teamId, scope] = key.split(':');
    return { teamId: Number(teamId), scope: scope as CardScope, rows };
  });
}

function evidenceGroupsToMap(groups: CardTeamEvidenceGroup[]) {
  return new Map(groups.map((group) => [`${group.teamId}:${group.scope}`, group.rows]));
}

function countReturnedEvidenceRows(groups: CardTeamEvidenceGroup[]) {
  return groups.reduce((total, group) => total + group.rows.length, 0);
}

function buildCardsHref(filters: CardFilters, teamId: number | null) {
...
function toCardTeamPanel({
  ...
  marketKey: string;
  marketLabel: string;
  matchRowsByTeamScope: Map<string, CardMatchRow[]>;
  ...
  return {
    ...
    nextFixture: row.next_fixture_id !== null && row.next_fixture_date !== null && row.next_opponent_team_id !== null && row.next_opponent_name !== null && row.next_venue_scope !== null
      ...
      : null,
    evidenceLoaded,
    evidenceHref: buildCardsHref(filters, row.team_id),
    summaryHref: buildCardsHref({ ...filters, teamSearch: '', teamId: null }, null),
  };
}

function metricFromQuickSnapshot(row: MarketTeamRankingRow, formWindow: CardFormWindow): CardQuickMetric {
  if (formWindow === 'last5') {
...
function opponentMetricFromQuickSnapshot(row: MarketTeamRankingRow, formWindow: CardFormWindow): CardQuickMetric | null {
...
function venueMatchesScope(scope: CardScope, fixture: CardNextFixture | null) {
  if (!fixture || scope === 'overall') return true;
  return scope === 'home' ? fixture.isHome : !fixture.isHome;
}

function fixtureMatchesFilter(scope: CardScope, fixture: CardNextFixture | null, fixtureFilter: CardFixtureFilter) {
  ...
  const targets: Record<Exclude<CardFixtureFilter, 'all' | 'with_fixture'>, string> = {
    today,
...
function buildComparisonHref(fixtureId: number, marketKey: string) {
  return `/comparison?fixtureId=${fixtureId}&marketKey=${marketKey}&source=cards-quick`;
}

function scopeTitle(scope: CardScope) {
  if (scope === 'home') return 'Home Matches';
  if (scope === 'away') return 'Away Matches';
  return 'All Matches';
}

export function parseCardFilters(searchParams: CardSearchParams | undefined, options: CardFilterOptions): CardFilters {
  const marketKeySelection = statisticFromCardMarketKey(firstParam(searchParams?.marketKey) ?? '');
  ...
  return {
    ...
    season,
    statistic,
    line: normalizeCardLine(statistic, marketKeySelection?.line ?? firstParam(searchParams?.line)),
...
export function parseCardQuickFilters(searchParams: CardSearchParams | undefined, options: CardFilterOptions): CardQuickFilters {
  const marketKeySelection = statisticFromCardMarketKey(firstParam(searchParams?.marketKey) ?? '');
  const statistic = marketKeySelection?.statistic ?? parseStatistic(searchParams?.statistic);
  const requestedLeagueId = parseInteger(searchParams?.leagueId) ?? options.defaultLeagueId;
  const requestedSeason = parseInteger(searchParams?.season) ?? options.defaultSeason;
...
const loadMarketDefinition = unstable_cache(async (marketKey: string) => {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .eq('category', 'cards')
    .maybeSingle();
  if (error) throw new Error(`Failed to load card market definition: ${error.message}`);
  return toMarketDefinitionRows(data ? [data] : [])[0] ?? null;
}, ['card-market-definition'], { revalidate: 300 });

async function loadCardFilterOptionsUncached(): Promise<CardFilterOptions> {
  const supabaseAdmin = getSupabaseAdmin();
  const [{ data: supportedData, error: supportedError }, { data: marketData, error: marketError }] = await Promise.all([
    ...
    supabaseAdmin
      .from('market_definitions')
      .select('key, label, is_active')
      .in('key', allCardMarketKeys())
      .eq('category', 'cards'),
  ]);
  if (supportedError) throw new Error(`Failed to load card league options: ${supportedError.message}`);
  if (marketError) throw new Error(`Failed to load card market options: ${marketError.message}`);
  ...
  const { data: leagueData, error: leagueError } = await supabaseAdmin
    ...
    .in('id', leagueIds);
  if (leagueError) throw new Error(`Failed to load card leagues: ${leagueError.message}`);
  ...
  const linesByStatistic = Object.fromEntries(
    CARD_STATISTIC_OPTIONS.map((option) => [
      option.value,
      CARD_LINES_BY_STATISTIC[option.value].filter((line) => activeKeys.has(resolveCardMarketKey(option.value, line))),
    ]),
  ) as Record<CardStatistic, string[]>;

  for (const option of CARD_STATISTIC_OPTIONS) {
  ...
  return {
    ...
    leagueSeasons,
    linesByStatistic,
    statistics: CARD_STATISTIC_OPTIONS,
  };
}

export const loadCardFilterOptions = unstable_cache(loadCardFilterOptionsUncached, ['card-filter-options'], { revalidate: 300 });

const loadCardProfileSnapshotRows = unstable_cache(
  async (marketKey: string, leagueId: number, season: number, teamId: number | null, teamSearch: string) => {
    ...
    const { data, error } = await query
      ...
      .limit(MAX_CARD_TEAM_PANELS);

    if (error) {
      throw new Error(`Failed to load card Market Serving Layer profiles. Run rebuild_market_serving_layer.py --category cards. Supabase: ${error.message}`);
    }

    return toProfileRows((data ?? []) as unknown[]);
  },
  ['card-profile-snapshot-rows'],
  { revalidate: MARKET_SERVING_CACHE_SECONDS },
);

const loadCardEvidenceSnapshotRows = unstable_cache(
  async (marketKey: string, leagueId: number, season: number, viewMode: CardViewMode, teamIdsKey: string) => {
    ...
    if (error) {
      throw new Error(`Failed to load card Market Serving Layer evidence. Run rebuild_market_serving_layer.py --category cards. Supabase: ${error.message}`);
    }

    return toEvidenceRows((data ?? []) as unknown[]);
  },
  ['card-evidence-snapshot-rows'],
  { revalidate: MARKET_SERVING_CACHE_SECONDS },
);

const loadCardQuickRankingSnapshotRows = unstable_cache(
  async (marketKey: string, leagueId: number, season: number) => {
    ...
    if (error) {
      throw new Error(`Failed to load card quick Market Serving Layer. Run rebuild_market_serving_layer.py --category cards. Supabase: ${error.message}`);
    }

    return toRankingRows((data ?? []) as unknown[]);
  },
  ['card-quick-ranking-snapshot-rows'],
  { revalidate: MARKET_SERVING_CACHE_SECONDS },
);

export async function loadCardMatchEvidence(
  filters: CardFilters,
  teamIds: number[],
): Promise<{ result: CardMatchEvidenceResult; timing: CardDetailTiming }> {
  const startedAt = Date.now();
  const marketKey = resolveCardMarketKey(filters.statistic, filters.line);
  ...
  const evidenceRows = await loadCardEvidenceSnapshotRows(
    marketKey,
    filters.leagueId,
    filters.season,
    filters.viewMode,
...
export async function loadCardTeamPanelsWithTiming(
  filters: CardFilters,
  options: { includeEvidence?: boolean } = {},
): Promise<{ result: CardDetailResult; timing: CardDetailTiming }> {
  const startedAt = Date.now();
  const marketKey = resolveCardMarketKey(filters.statistic, filters.line);
  ...
  const [marketDefinition, profileRows] = await Promise.all([
    ...
    loadCardProfileSnapshotRows(marketKey, filters.leagueId, filters.season, filters.teamId, filters.teamSearch.trim()),
  ]);
  ...
  let evidenceGroups: CardTeamEvidenceGroup[] = [];
  let dbEvidenceMs = 0;
  let evidenceRowCount = 0;
  let returnedEvidenceRowCount = 0;

  if (includeEvidence) {
    const evidence = await loadCardMatchEvidence(filters, profileRows.map((row) => row.team_id));
  ...
  const panels = profileRows.map((row) => toCardTeamPanel({
...
export async function loadCardTeamPanels(filters: CardFilters): Promise<CardDetailResult> {
  const { result } = await loadCardTeamPanelsWithTiming(filters);
  return result;
}

export async function loadCardQuickScanner(filters: CardQuickFilters): Promise<CardQuickResult> {
  const { result } = await loadCardQuickScannerWithTiming(filters);
  return result;
}

export async function loadCardQuickScannerWithTiming(
  filters: CardQuickFilters,
): Promise<{ result: CardQuickResult; timing: CardQuickTiming }> {
  const startedAt = Date.now();
  const marketKey = resolveCardMarketKey(filters.statistic, filters.line);
  const dbStartedAt = Date.now();
  const [marketDefinition, snapshotRows] = await Promise.all([
    loadMarketDefinition(marketKey),
    loadCardQuickRankingSnapshotRows(marketKey, filters.leagueId, filters.season),
  ]);
  const dbMs = Date.now() - dbStartedAt;
  const transformStartedAt = Date.now();
  const marketLabel = marketDefinition?.label ?? fallbackMarketLabel(filters.statistic, filters.line);
  const normalizedSearch = filters.teamSearch.trim().toLowerCase();
  const candidatesByScope = new Map<CardScope, CardQuickRow[]>();
  ...
  const columns: CardQuickColumn[] = SCOPES.map((scope) => {
  ...
  const emergingColumns: CardQuickColumn[] = SCOPES.map((scope) => {
    const rows = (candidatesByScope.get(scope) ?? [])
